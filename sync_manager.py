...
class SyncManager:
    ...
    def _set_folder(self, which: str, value):
        ...
        if not os.path.exists(value):
            logger.info(f"Folder {which.upper()} does not exist and is being created: \"{value}\"")
            os.makedirs(value)
        setattr(self, f"_folder_{which.lower()}", value)
        logger.info(f"Folder {which.upper()} set to: \"{value}\"")

    # def load_history(self):
    #     try:
    #         if not os.path.exists(self._history_file):
    #             logger.info(f"No history file found at \"{self._history_file}\". A new one will be created.")
    #             self._history = {}
    #             return
    #         with open(self._history_file, 'r') as f:
    #             self._history = json.load(f)
    #             logger.info(f"History file loaded \"{self._history_file}\"")
    #     except Exception as e:
    #         logger.error(f"Error loading history from file \"{self._history_file}\": {str(e)}")
    #         raise e

    # def save_history(self):
    #     with open(self._history_file, 'w') as f:
    #         json.dump(self._history, f, indent=4)
    #     logging.info(f"History saved to file \"{self._history_file}\"")
    ...
    @staticmethod
    def _copy_file(src_folder: str, dest_folder: str, file_relpath: str):
        src_path = os.path.join(src_folder, file_relpath)
        dst_path = os.path.join(dest_folder, file_relpath)
        try:
            os.makedirs(os.path.dirname(dst_path), exist_ok=True)
        except Exception as e:
            logger.error(f"Error creating folder: {os.path.dirname(dst_path)}")
        try:
            shutil.copy2(src_path, dst_path)
        except Exception as e:
            logger.error(f"Error copying file: \"{src_path}\" -> \"{dst_path}\"")
    ...
    def _create_folder(folder: str, folder_relpath: str):
        ...
        if not os.path.exists(folder_path):
            try:
                ...
                logger.error(f"Error creating folder: \"{folder_path}\"")
    ...
    def _handle_conflict(self, file_relpath, file_a, file_b):
        ...
        timestamp_a = datetime.fromtimestamp(file_a["mtime"]).strftime("%Y%m%d_%H%M%S")
        timestamp_b = datetime.fromtimestamp(file_b["mtime"]).strftime("%Y%m%d_%H%M%S")
        relpath, ext = os.path.splitext(file_relpath)
        conflict_name_a = f"{relpath}_conflictA_{timestamp_a}{ext}"
        conflict_name_b = f"{relpath}_conflictB_{timestamp_b}{ext}"
        # Copy from A to A and B
        shutil.copy2(str(os.path.join(self.folder_a, file_relpath)), str(os.path.join(self.folder_a, conflict_name_a)))
        shutil.copy2(str(os.path.join(self.folder_a, file_relpath)), str(os.path.join(self.folder_b, conflict_name_a)))
        # Copy from B to A and B
        shutil.copy2(str(os.path.join(self.folder_b, file_relpath)), str(os.path.join(self.folder_a, conflict_name_b)))
        shutil.copy2(str(os.path.join(self.folder_b, file_relpath)), str(os.path.join(self.folder_b, conflict_name_b)))
        # Delete original files
        os.remove(os.path.join(self.folder_a, file_relpath))
        os.remove(os.path.join(self.folder_b, file_relpath))
